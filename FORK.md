...
| Make `GracefulStop`'s flag atomic | `isGracefulStop` was a plain `bool` written by the `readUp` goroutine (which consumes the `GracefulStop` signal) and read by `runMigrations` on the caller's goroutine; because `readUp` runs `PrefetchMigrations` ahead, the two touch it concurrently and `go test -race` reports it for any stop that lands mid-run. Now an `atomic.Bool`. `TestGracefulStopConcurrent` reproduces the interleaving. Candidate for upstream. |
